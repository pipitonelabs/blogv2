...
description: "A detailed walkthrough of how I switched from the self-hosted Waline comment system to GitHub-powered Giscus."
tags:
  - Astro
  - Web Development
  - Tutorial
...
Waline is genuinely impressive. It supports multiple databases, has a beautiful UI and offers features like emoji reactions and comment management. But the deployment seemed more like a burden than a benefit for my use case. I also wanted to utilize a solution that was US-based.
...
Head over to [giscus.app](https://giscus.app) and configure your settings. You'll need to:

1. Enter your repository (e.g., `yourusername/blog`)
2. Choose a discussion category (I recommend creating a dedicated "Comments" category)
3. Select your mapping preference (I use `pathname` so each page gets its own discussion)
4. Pick your theme and features

The site will generate all the values you need. Keep that page open - you'll need those IDs in a moment.
...
- We're pulling configuration from a centralized config file (more on that next)
...
Don't forget to export the component. Create an [`index.ts`](src/components/giscus/index.ts:1) file:
...
This is where all the magic values from giscus.app go. In my [`site.config.ts`](src/site.config.ts:167), I added the Giscus configuration:
...
<Aside type="caution" title="Keep Your IDs Safe">
  The `repoId` and `categoryId` are not secrets per se, but they're unique to your repository. Get them directly from the [giscus.app](https://giscus.app) configuration page for your repo.
</Aside>

## Step 4: Add Type Definitions

If you're using TypeScript, you'll want proper types for the Giscus config. Here's the schema I added to my [`integrations-config.ts`](packages/pure/types/integrations-config.ts:49):
...
This gives you autocomplete and type checking for all your Giscus options.

## Step 5: Integrate with Your Blog Layout

The final step is adding the Comment component to your blog post layout. In my [`BlogPost.astro`](src/layouts/BlogPost.astro:11) layout, I import and use the component:
...
Notice the conditional: comments only appear on published posts (`!isDraft`) and when the individual post has comments enabled (`enableComment`). This gives you granular control - you can disable comments on specific posts by setting `comment: false` in the frontmatter.

## Step 6: Remove Waline (The Bittersweet Part)

With Giscus working, it was time to say goodbye to Waline:

1. **Remove the Waline components**  -  Delete your Waline component files
2. **Remove Waline dependencies** — `bun remove @waline/client`
3. **Clean up config**  -  Remove Waline-related configuration
4. **Shut down the backend**  -  Decommission your Waline server/function
